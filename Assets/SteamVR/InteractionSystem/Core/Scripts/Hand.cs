...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        /*
        public enum TrollStages
        {
            LegSize = 1, // Stage 1
            HumanSize = 2, // Stage 2
            BigAnimalSize = 3, // Stage 3
            TreeSize = 4, // Stage 4
            MountainKing = 5, // Stage 5
        };
        */

        [Header("Variables I made and I know what they do")]
        public Camera thisCamera;
        [HideInInspector]
        public bool overYourHead;

        public Animator _animator;

        public bool isHolding;

        public Edible.Edible edible;

        private PlayerBehaviour _player;

        //-------------------------------------------------------------------------------------------------------
        [Header("Steam VR Variables")]
        ...
        [SteamVR_DefaultAction("MenuPress")]
        ...
        {
            if (objectToAttach.GetComponent<NavMeshAgent>())
            {
                Debug.Log("Animal in hand");
                objectToAttach.GetComponent<NavMeshAgent>().enabled = false;
            }

            //if (objectToAttach.GetComponent<AIWildAnimals>())
            //{
            //    objectToAttach.GetComponent<AIWildAnimals>().enabled = false;
            //}

            if (!objectToAttach.GetComponent<Interactable>()) return;
            if (objectToAttach.GetComponent<Edible.Edible>()._edibleIndex > (int)_player.stage) return;
            if (!objectToAttach.GetComponent<ItemController>()) return;

            

            objectToAttach.GetComponent<ItemController>().interactedWith = true;
            objectToAttach.GetComponent<Rigidbody>().constraints = RigidbodyConstraints.None;
            isHolding = true;
        ...
        public void DetachObject(GameObject objectToDetach, bool restoreOriginalParent = true)
        {
            ...
            if (objectToDetach.GetComponent<NavMeshAgent>())
            {
                Debug.Log("Animal NOT hand");
                objectToDetach.GetComponent<NavMeshAgent>().enabled = false;
            }

            /*
            if (objectToDetach.GetComponent<AIWildAnimals>())
            {
                objectToDetach.GetComponent<AIWildAnimals>().enabled = false;
            }
            */
            Debug.Log("I am " + isHolding);
            ...
            {
                Debug.Log("Let it go");
                objectToDetach.GetComponent<NavMeshAgent>().enabled = true;
            }
        }
        ...
        protected virtual IEnumerator Start()
        {
            ...
            _player = PlayerBehaviour._instance;
            if(!_player) Debug.LogError("No player in sight");
        ...
        protected virtual void Update()
        {
            ...
            //Debug.Log(_animator.GetBool("Grab"));

            if(transform.position.y > thisCamera.transform.position.y)
            {
                overYourHead = true;
                Debug.Log("Did I just go over your head??");
            }
            else
            {
                overYourHead = false;
